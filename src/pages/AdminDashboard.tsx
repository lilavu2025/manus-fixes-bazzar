  ...
  return (
    <div className={`min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 ${isRTL ? 'rtl' : 'ltr'}`} dir={isRTL ? 'rtl' : 'ltr'}>
      <div className="flex flex-col lg:flex-row min-h-screen">
        {/* Sidebar */}
        <aside className={`z-50 ${sidebarOpen ? 'fixed inset-0 bg-black/40' : ''} lg:static lg:bg-transparent`}>
          <div className={`transition-all duration-300 bg-white shadow-2xl border-r border-gray-200 flex flex-col min-h-screen fixed lg:relative ${sidebarCollapsed ? 'lg:w-20' : 'lg:w-72'} w-72 ${sidebarOpen ? 'translate-x-0' : isRTL ? 'translate-x-full' : '-translate-x-full'} lg:translate-x-0`}>
        ...
        <main className="flex-1 min-w-0 flex flex-col">
          {/* Mobile Header */}
          <header className="lg:hidden bg-white border-b border-gray-200 p-4 flex items-center justify-between sticky top-0 z-40 shadow-sm">
          ...
          {location.pathname === "/admin" && (
            <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6 p-4 lg:p-8 max-w-7xl mx-auto w-full">
              <AdminDashboardStats 
          ...
          {/* Content */}
          <section className="flex-1 p-4 lg:p-8 overflow-x-auto max-w-7xl mx-auto w-full">
            <Routes>
