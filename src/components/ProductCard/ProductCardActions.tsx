  ...
  return (
    <div className="absolute top-2 left-2 flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-300 transform -translate-x-4 group-hover:translate-x-0 transition-transform">
      ...
      <Popover open={shareOpen} onOpenChange={setShareOpen}>
        ...
        <PopoverContent align="end" className="w-64 max-w-xs p-2 space-y-1 z-50 bg-white border border-gray-200 rounded-lg shadow-xl animate-fade-in">
          <div className="flex flex-col gap-1">
            <button className="flex items-center gap-2 w-full px-2 py-1 hover:bg-gray-100 rounded text-sm transition-colors" onClick={handleShareWhatsapp}>
              <MessageCircle className="h-4 w-4 text-green-600" />
              {t('shareViaWhatsapp')}
            </button>
            <button className="flex items-center gap-2 w-full px-2 py-1 hover:bg-gray-100 rounded text-sm transition-colors" onClick={handleShareEmail}>
              <Mail className="h-4 w-4 text-blue-600" />
              {t('shareViaEmail')}
            </button>
            <button className="flex items-center gap-2 w-full px-2 py-1 hover:bg-gray-100 rounded text-sm transition-colors" onClick={handleCopyLink}>
              <Copy className="h-4 w-4" />
              {t('copyLink')}
            </button>
            {navigator.share && (
              <button className="flex items-center gap-2 w-full px-2 py-1 hover:bg-gray-100 rounded text-sm transition-colors" onClick={handleNativeShare}>
