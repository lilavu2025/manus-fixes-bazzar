...
// Normalize variant attributes (object or JSON string)
...
const OrderAddDialog: React.FC<OrderAddDialogProps> = ({
  ...
  // Helper: compute unit price for a product item respecting variant selection and user type
  const computeUnitBasePrice = useCallback((product: any, item: any, userType: string) => {
    if (!product) return 0;
    // If variant selected, prefer variant price
  ...
  useEffect(() => {
    ...
    setOrderForm(prev => {
      const items = prev.items.map(item => {
        ...
        if (!p) return item;
  const base = computeUnitBasePrice(p, item, userType);
  ...
  const basePriceForProduct = computeUnitBasePrice(p, it, userType);
                ...
                <div className="text-right mt-3 space-y-2">
                  {(() => {
                    ...
                    const subtotalBeforeDiscounts = orderForm.items
                      ...
                      .reduce((sum, it: any) => {
                        const p = productOf(products, it.product_id);
                        // Prefer original_price if present; else compute variant-aware base
                        const unit = typeof it.original_price === 'number' && it.original_price > 0
                          ? it.original_price
                          : computeUnitBasePrice(p, it, userType);
