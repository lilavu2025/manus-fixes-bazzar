...
const UserActivityLogTable: React.FC = () => {
  ...
  const exportAdminActivityToExcel = () => {
    const ws = XLSX.utils.json_to_sheet(logs.map(l => {
      ...
      if ((!userName || userName === l.user_id) && l.details && typeof l.details === 'object') {
        const detailsObj = l.details as { full_name?: string; email?: string; phone?: string };
      ...
      if ((!adminName || adminName === l.admin_id) && l.details && typeof l.details === 'object') {
        const detailsObj = l.details as { admin_full_name?: string; admin_email?: string; admin_phone?: string };
        adminName = detailsObj.admin_full_name || adminName || 'أدمن غير متوفر';
    ...
    saveAs(new Blob([wbout], { type: 'application/octet-stream' }), 'admin-activity.xlsx');
  };
