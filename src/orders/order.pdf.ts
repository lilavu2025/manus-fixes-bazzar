...
export async function downloadInvoicePdf(
  ...
  const freeItemsRows = (() => {
    try {
      ...
      // 2) اجلب من order.free_items (يحتوي عادةً variantAttributes)
      const fromOrder: any[] = (() => {
      ...
      // 3) دمج القائمتين وتفضيل السجلات التي تحتوي على variantAttributes
      ...
      const merge = (target: any, src: any) => {
        // انسخ الحقول الناقصة فقط، ولاحظ تفضيل variantAttributes إن وجدت
        const result: any = { ...target };
        for (const k of Object.keys(src || {})) {
          if (result[k] == null || result[k] === '') result[k] = src[k];
        }
        // خصّص تفضيل variantAttributes
      ...
      const pushOrMerge = (arr: any[]) => {
        for (const it of arr) {
          ...
          if (!key.startsWith('|')) { // لديه productId
            if (map.has(key)) {
              map.set(key, merge(map.get(key), it));
            } else {
              map.set(key, { ...it });
            }
          }
      ...
      // ضع من offers أولاً ثم من order ليقوم الأخير بإثراء السجلات بالفيرينتس
